...
export class UserActivityLogService {
    ...
    async log(userActivityTypeId: string, userId: string, date: Date, os: string, osVersion: string, browser: string) {
      try {
        ...
        return await this.userActivityLogRepo.manager.transaction(async (entityManager) => {
            ...
            } else if(Number(userActivityTypeId) === Number(ActivityTypeEnum.USER_LOGOUT)) {
    ...
    async userLogin(entityManager: EntityManager, userId: string, date: Date, os: string, osVersion: string, browser: string) {
        ...
        if(lastActivitySameDevice[0] && lastActivitySameDevice[0].userActivityType.userActivityTypeId === ActivityTypeEnum.USER_LOGOUT.toString() 
        && (!lastActivityOtherDevice[0] && lastActivityOtherDevice[0] === undefined)) {
            userActivityLog = new UserActivityLog();
            userActivityLog.userActivityType = await entityManager.findOne(UserActivityType, {where: { userActivityTypeId: userActivityTypeId }})
            userActivityLog.user = await entityManager.findOne(Users, {where: { userId: userId }})
            userActivityLog.date = date;
            userActivityLog.os = os;
            userActivityLog.osVersion = osVersion;
            userActivityLog.browser = browser;
            userActivityLog = await entityManager.save(UserActivityLog, userActivityLog);
        } else if(lastActivitySameDevice[0] && lastActivitySameDevice[0].userActivityType.userActivityTypeId === ActivityTypeEnum.USER_LOGOUT.toString() 
        && lastActivityOtherDevice[0] && lastActivityOtherDevice[0].userActivityType.userActivityTypeId === ActivityTypeEnum.USER_LOGIN.toString() ) {
            userActivityLog = new UserActivityLog();
            userActivityLog.userActivityType = await entityManager.findOne(UserActivityType, {where: { userActivityTypeId: ActivityTypeEnum.USER_LOGOUT.toString()  }})
            userActivityLog.user = await entityManager.findOne(Users, {where: { userId: userId }})
            userActivityLog.date = date;
            ...
            userActivityLog.browser = browser;
            userActivityLog = await entityManager.save(UserActivityLog, userActivityLog);
        } else if(lastActivitySameDevice[0] && lastActivitySameDevice[0].userActivityType.userActivityTypeId === ActivityTypeEnum.USER_LOGIN.toString()) {
        ...
        && lastActivityOtherDevice[0].userActivityType.userActivityTypeId === ActivityTypeEnum.USER_LOGOUT.toString()) {
            userActivityLog = new UserActivityLog();
            userActivityLog.userActivityType = await entityManager.findOne(UserActivityType, {where: { userActivityTypeId: userActivityTypeId }})
            userActivityLog.user = await entityManager.findOne(Users, {where: { userId: userId }})
            userActivityLog.date = date;
        ...
        && (lastActivityOtherDevice[0].os !== os || lastActivityOtherDevice[0].osVersion !== osVersion || lastActivityOtherDevice[0].browser !== browser)
        && lastActivityOtherDevice[0].userActivityType.userActivityTypeId === ActivityTypeEnum.USER_LOGIN.toString()) {
            userActivityLog = new UserActivityLog();
            userActivityLog.userActivityType = await entityManager.findOne(UserActivityType, {where: { userActivityTypeId: ActivityTypeEnum.USER_LOGOUT.toString() }})
            userActivityLog.user = await entityManager.findOne(Users, {where: { userId: userId }})
            userActivityLog.date = date;
    ...
    async userLogout(entityManager: EntityManager, userId: string, date: Date, os: string, osVersion: string, browser: string) {
        ...
        if(lastActivitySameDevice[0] && lastActivitySameDevice[0].userActivityType.userActivityTypeId === ActivityTypeEnum.USER_LOGIN.toString()) {
            userActivityLog = new UserActivityLog();
            userActivityLog.userActivityType = await entityManager.findOne(UserActivityType, {where: { userActivityTypeId: userActivityTypeId }})
            userActivityLog.user = await entityManager.findOne(Users, {where: { userId: userId }})
            userActivityLog.date = date;
